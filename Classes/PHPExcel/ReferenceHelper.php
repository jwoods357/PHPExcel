...
class PHPExcel_ReferenceHelper
{
    ...
    protected function adjustPageBreaks(PHPExcel_Worksheet $pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
    {
        ...
        ($pNumCols > 0 || $pNumRows > 0) ?
            uksort($aBreaks, array(\PHPExcel_ReferenceHelper::class,'cellReverseSort')) :
            uksort($aBreaks, array(\PHPExcel_ReferenceHelper::class,'cellSort'));
    ...
    protected function adjustHyperlinks($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
    {
        ...
        ($pNumCols > 0 || $pNumRows > 0) ? uksort($aHyperlinkCollection, array(\PHPExcel_ReferenceHelper::class,'cellReverseSort')) : uksort($aHyperlinkCollection, array(\PHPExcel_ReferenceHelper::class,'cellSort'));
    ...
    protected function adjustDataValidations($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
    {
        ...
        ($pNumCols > 0 || $pNumRows > 0) ? uksort($aDataValidationCollection, array(\PHPExcel_ReferenceHelper::class,'cellReverseSort')) : uksort($aDataValidationCollection, array(\PHPExcel_ReferenceHelper::class,'cellSort'));
    ...
    protected function adjustProtectedCells($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
    {
        ...
        ($pNumCols > 0 || $pNumRows > 0) ?
            uksort($aProtectedCells, array(\PHPExcel_ReferenceHelper::class,'cellReverseSort')) :
            uksort($aProtectedCells, array(\PHPExcel_ReferenceHelper::class,'cellSort'));
    ...
    private function updateCellRange($pCellRange = 'A1:A1', $pBefore = 'A1', $pNumCols = 0, $pNumRows = 0)
    {
        if (strpos($pCellRange, ':') !== false || strpos($pCellRange, ',') !== false) {
            ...
            $ic = count($range);
            for ($i = 0; $i < $ic; ++$i) {
                $jc = is_array($range[$i]) || $range[$i] instanceof \Countable ? count($range[$i]) : 0;
                for ($j = 0; $j < $jc; ++$j) {
